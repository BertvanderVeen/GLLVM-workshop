...
\begin{itemize}
\item Fixed effects are \textbf{maximised}: the likelihood is optimised directly over $\hat{\boldsymbol{\beta}}$
\item Random effects are \textbf{marginalised}: $\textbf{u}$ is integrated out of the likelihood
\end{itemize}

\pause

Random effects are stochastic: like the data, they come from a distribution $\mathbf{u} \sim \mathcal{N}(\mathbf{0}, \boldsymbol{\Sigma})$.  \textcolor{red}{A \textbf{mixed-effects model} contains both.}
...
LHS determines the size of $\boldsymbol{\Sigma}$; RHS the number of draws per $1 \ldots n$ site ($p$ species).
...
\begin{tabular}{lccc}
\toprule
Formula & $\boldsymbol{\Sigma}$ & REs per site & Total REs \\
\midrule
