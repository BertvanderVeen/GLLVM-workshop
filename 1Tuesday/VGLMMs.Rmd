...
Remember: we have 13 species. Enough for variances, not for correlations.
...
- \texttt{glmmVA} collapses long-format data to a univariate \texttt{gllvm} call, and calls \texttt{gllvm} internally wrapping \texttt{row.eff}
...
Internally, \texttt{glmmVA} reduces $y$ to a single column: each "row" is one site$\times$species observation, not a site. \texttt{Species} is then just another row-level covariate, so it is legal (if unusual) to see it inside \texttt{row.eff}:

\footnotesize

```{r bridge, eval=FALSE}
glmmVA(Count ~ Species + (0+NO3|Species), ...)
# strips the response, passes the formula as row.eff on the collapsed (site x species) rows:
gllvm(y, studyDesign=X, row.eff=~Species+(0+NO3|Species), ...)
```
...
On the genuine wide-format response matrix (species as columns, sites as rows), this trick does not apply: \texttt{Species} can never appear in \texttt{row.eff}, since rows are sites only. Species-specific effects instead use \texttt{formula}, the wide-format equivalent:
...
\texttt{|Species} (long format, collapsed rows) $\equiv$ \texttt{|1} (wide format, per column): one $\mathcal{N}(\mu, \sigma^2)$ for all species. \newline
\texttt{formula} gives species-specific $\beta_j$ on the real (wide) response matrix; \texttt{row.eff} gives site-level effects shared across species.
...
m_rep <- gllvm(fy, X=fX,
               ...
               formula=~(0+DBH.CM|1),
               row.eff=~(1|RESERVE),
               family="binomial", link="logit", num.lv=0,
               beta0com=FALSE)
```
