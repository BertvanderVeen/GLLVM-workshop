...
\textcolor{red}{\textbf{There has been no successful ordination method that explicitly incorporates a response model}}
...
```{r, echo = FALSE, fig.height = 3.5, message=FALSE, results = FALSE}
unimodal <-function(x, opt, tol)exp(1-0.5*(x-opt)^2/tol^2)

plot(NA, type="n", ylim=c(0, exp(1)),yaxs="i",yaxt="n",xaxt="n",xlab="Niche dimension 1", ylab="Frequency", cex.lab=1.5,cex.axis=1.5, xlim = c(-2,2))
...
```{r, echo = FALSE, fig.height = 6, results = FALSE, warning=FALSE, message=FALSE, fig.width = 10}
par(mfrow=c(2,3))
unimodal <-function(x, opt, tol)exp(1-0.5*(x-opt)^2/tol^2)
...
- The most complex ecological ordination method to date
...
## How to interpret the ordination plot
...
```{r, echo = FALSE, fig.height = 6, fig.cap = "Species effects as increase direction"}
mu1 <- -2
mu2 <- 1
mu11 <- 2
mu21 <- 1.1
...
```{r, echo = FALSE, fig.height = 6, fig.cap = "Species effects as centroids"}
mu1 <- -2;    sd1 <- 2
mu2 <- 1;    sd2 <- 1
mu11 <- 2;  sd11 <- 1
mu21 <- 1.1; sd21 <- 0.5
...
```{r, message = FALSE, fig.height = 7, out.width = "49%", echo = -c(1:2), fig.show  = "hold"}
...
```{r spid1, fig.height = 6, dev = "png", fig.show = "hide", echo = -4}
...
\columnsbegin
\column{0.5\textwidth}
\includegraphics[width=\linewidth]{Unimodal_files/figure-beamer/spid1-1.png}
\column{0.5\textwidth}
\alt<2>{
\includegraphics[width=\linewidth]{Unimodal_files/figure-beamer/spid1-3.png}
}{
\includegraphics[width=\linewidth]{Unimodal_files/figure-beamer/spid1-2.png}
}
...
par(mar=c(5,4,1,2))
curve(dnorm,from=-2,to=2, main = "Std. normal distribution", cex.main = 0.8)
```
...
If we misspecify the model as linear, we can miss the dominant gradient altogether.
...
model3 <- gllvm(Y, num.lv = 2, family = "binomial", quadratic = TRUE, sd.errors = FALSE, optim.method = "L-BFGS-B", seed = 2)
...
```{r, message=FALSE, echo = FALSE, fig.height = 3.2, warning=FALSE}
par(mfrow=c(2,4), mar=c(2,2,2,1))
PCA <- prcomp(PM1)
vegan::ordiplot(PCA, type = "text", display = "sites", cex = 1.2, main = "PCA")
CA <- vegan::cca(PM1)
vegan::ordiplot(CA, type = "text", cex = 1.2, display = "sites", main = "CA")
DCA <- vegan::decorana(PM1)
vegan::ordiplot(DCA, type = "text", display = "sites", cex = 1.2, main = "DCA")
plot.new()
NMDS <- vegan::metaMDS(PM1, k =1, trace = 0)
plot(NMDS$points, xlab = "Index", ylab = "Dim 1", main = "1D NMDS", cex.main = 1)
NMDS <- vegan::metaMDS(PM1, k =2, trace = 0)
plot(NMDS$points, xlab = "Index", ylab = "Dim 1", main = "2D NMDS", cex.main = 1)
PCoA1 <- cmdscale(vegan::vegdist(PM1))
PCoA2 <- cmdscale(vegan::vegdist(PM1, method = "gower"))
vegan::ordiplot(PCoA1, type = "text", main = "PCoA Bray", cex.main = 1)
vegan::ordiplot(PCoA2, type = "text", main = "PCoA Canberra", cex.main = 1)
```
...
Successfully fitting a quadratic model usually takes a few runs, it tends to be numerically less stable than an ordinary GLLVM.
...
```{r, echo=F, message=F, warning=F, fig.height = 6, echo = FALSE}
library(vegan)
