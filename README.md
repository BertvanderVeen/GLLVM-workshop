...
- add a comparison to copulas (e.g. ecoCopula) on Friday, for the mixed-responses material. A copula maps each species' points through its own marginal CDF to a Gaussian, for every species, then estimates correlation among those transformed variables -- so the correctness of the estimated correlations depends strongly on correctly specifying every marginal, and uncertainty is generally not available (typically a two-stage/plug-in procedure, not jointly estimated). GLLVM instead estimates a blend of correlation on the link scale via the shared latent variables: it is a one-stage joint estimator with proper uncertainty, but the interpretation differs across species pairs and is a blend of whatever link functions those species use, which does not necessarily correspond to the same point/scale across pairs.
- in the spatial/temporal/phylogenetic confounding part of Friday's Extensions material (the last part before the case study), add plotting residuals against space or time as a diagnostic option for spotting this kind of confounding, alongside the existing theoretical explanation.
-->
