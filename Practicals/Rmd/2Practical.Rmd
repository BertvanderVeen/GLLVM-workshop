...
`AIC()` works for comparing models fitted with the same method, but there is a subtlety: variance components should be compared with REML, fixed effects with ML.

**Questions:**

3. Compare two models that differ in a fixed effect using both ML and REML. Does the ranking change? Which should you trust?
...
Residual diagnostics for VGLMMs are covered in depth in the **Checking** lecture and its accompanying practical. For now, a quick sanity check using simulation-based residuals:

```{r, eval=FALSE}
library(DHARMa)
# fit your model first, then:
res <- simulateResiduals(model)
plot(res)
```

For VGLMMs it is also worth checking the normality of estimated random effects:
...
5. Do your simulation-based residuals show any pattern? If so, what might be missing from the model?
