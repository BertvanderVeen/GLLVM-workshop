...
Today's lecture used the Alpine data (elevation gradient), the spider data (in <tt>mvabund</tt>), and the Podani/Minchin data (downloaded directly in the Rmd) to illustrate unimodal responses. Any dataset where you expect species to have optima along a gradient is suitable here, except beta-distributed responses (not yet supported for the quadratic model); besides the wadden data used below, you could use:

1. Alpine ("alpineX/Y", presence-absence, elevation gradient; used in the lecture)
2. spider (counts, in <tt>mvabund</tt> via `data(spider, package = "mvabund")`; used in the lecture)
3. Dune ("dune", percentage cover, in <tt>vegan</tt>; a classic case for unimodal-response ordination methods)
...
The `n.init` option is very necessary: the GLLVM with unimodal response is even more prone to finding a suboptimal solution than usual. That also means that fitting the model is much slower, so using <tt> gllvm</tt>'s (new) option for parallel computation could help. `ordiplot` will plot the species optima, and if the optima are too far away from the estimated latent variable, it will plot the species effects as arrows instead.
