---
title: "Hierarchically modelling species responses"
...
```{r teplot, echo = FALSE, dev = "png", fig.show = "hide", warning=FALSE, fig.height = 3,fig.width=5,fig.align="center",  dpi = 300}
# Load libraries
library(ggplot2)
...
\columnsbegin
\column{0.3\textwidth}
\includegraphics[width=\linewidth]{coldplant.jpg}
\column{0.3\textwidth}
\includegraphics[width=\linewidth]{HierarchicalResponses_files/figure-beamer/teplot-1.png}
\column{0.3\textwidth}
\includegraphics[width=\linewidth]{oxalis.jpg}
\columnsend
}{\includegraphics[height=0.55\paperheight, width = \linewidth]{HierarchicalResponses_files/figure-beamer/teplot-1.png}}
...
\pause
 ...
 \begin{figure}
 \includegraphics[scale=1.5]{snowshoe2.png}
...
- You can use \texttt{glm}, \texttt{glmmVA}, or \texttt{gllvm} to fit this
...
- 6 environmental variables: aspect, slope, microscale landform, physical disturbance, zoogenic disturbance (trampling/burrowing), and mean Julian snowmelt date
- 8 traits: height, spread, angle, area, thick, sla, N_mass, seed
...
beta <- sweep(model2$TR%*%t(Bxtr), 2, Bx, "+")
```
...
There are a lot more "free" parameters in the VGLM. The second model is either "constrained", or alternative; we provide it (much) more information.

# Hierarchical responses

## Accounting for unmeasured traits
...
\begin{equation}
\eta_{ij} = \beta_{0j} + \textbf{x}_i^\top(\tikzmarknode{t1}{\symbf{\beta}_x} + \tikzmarknode{t3}{\textbf{B}_{xtr}^\top}\tikzmarknode{t4}{\textbf{tr}_j} + \tikzmarknode{t2}{\textbf{b}_j})
\end{equation}
...
Niku et al. (2021) concluded that omitting $\textbf{b}_j$ leads to inflated Type I error (overly optimistic conclusions w.r.t.\ traits).
...
ter Braak (2023) concluded that fourth-corner models are often overconfident in small samples.

# Community Phylogenetics

## Trait evolution

Traits develop when they improve fitness.

- Related species tend to be similar due to shared evolutionary history
- Recent divergence means less time for trait differentiation
- Therefore, they are often found in similar environments
...
- In the 4th corner model $\textbf{b}_j$ can be structured by Phylogeny
...
## Phylogenetic random effects: the model
...
## Fitting large phylogenetic models

The full $m \times m$ covariance matrix $\textbf{C}$ is infeasible for large species sets.

\texttt{gllvm} uses a \textbf{Nearest Neighbour Gaussian Process (NNGP)}: each species conditions on its $k$ nearest neighbours on the tree (\texttt{nn.colMat = 15}). Species must be ordered by their phylogenetic position for the approximation to be valid.

\columnsbegin
\column{0.55\textwidth}
\includegraphics[height=0.5\textheight]{specord-1.png}
\column{0.45\textwidth}
\footnotesize \texttt{gllvm:::findOrder()} computes the ordering. See \href{https://jenniniku.github.io/gllvm/articles/vignette7.html}{vignette 7}.
\columnsend
...
## Example with fungi data: tree

\vspace{-2\baselineskip}

```{r tree, fig.align="center", fig.height = 4, fig.width = 4, echo = FALSE}
plot(tree, show.tip.label = FALSE)
```

## Example: ordering species for the NNGP
...
## Example with fungi data: model

\tiny

```{r fita, cache = TRUE, message = FALSE, warning=FALSE, results="hide"}
...
Fitting time scales with model complexity; the NNGP approximation keeps this tractable.

## Example with fungi data: summary
...
## Example with fungi data: correlated effects

\footnotesize

```{r, fig.height = 3, fig.width = 3.5, echo = FALSE, fig.align = "center"}
...
## Example with fungi data: \texttt{phyloplot}

\footnotesize

```{r heatmap, cache = TRUE, fig.height = 2.5, fig.width = 4, dpi = 150, fig.align="center"}
...
- Traits explain why species respond to the environment: the fourth corner
- Residual random effects account for unmeasured traits: hierarchical responses
- Phylogenetic random effects structure species responses by evolutionary history
- Large phylogenetic models are made feasible by the NNGP approximation
- All components combine in one model: traits, phylogeny, and environment
