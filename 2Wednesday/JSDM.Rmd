...
- Similar to the model from yesterday

<!--
## Co-occurrence of Swiss birds

```{r alphamap, eval = FALSE, dpi = 300, echo = FALSE, cache = TRUE, warning=FALSE,message=FALSE, fig.height = 7, dev = "png", fig.show="hide"}
...
plt<-ggplot() +         # layer 3
  theme_classic()+theme(text = element_text(size = 20),axis.text=element_blank(), axis.ticks=element_blank())+scale_fill_viridis_c(name="Dominance", na.value="white")+geom_raster(data=dat, aes(x=x, y=y, fill = value), interpolate=TRUE)+coord_fixed()+xlab(NULL)+ylab(NULL)+ggtitle("Dominance (due to abundance, aspect and slope)")
plt
```

```{r predmap, eval = FALSE, cache = TRUE, echo = FALSE, message = FALSE, warning=FALSE, dpi = 300, fig.width = 3, fig.height = 4.5, dev = "png", fig.show="hide"}
source("../1Tuesday/switzerland/slope.R")
X = cbind(1, 
...
\textbf{Can the distribution of one species inform us of other species'?}
...
\textcolor{red}{More on diversity prediction later.}
-->
...
\textcolor{red}{But, observations of the same species form groups. Co-occurring species have more similar observations to each other than to other species}
...
Niches are shaped by more than environment and interactions; historical limitations, dispersal, and other processes prevent us from estimating the fundamental niche.
...
Either way, it results in correlations between responses
...
- So we expect positive values if species co-occur, and negative if they do not
...
- The number of pairwise associations grows quadratically
  - 1 with 2 species, 6 for 4 species, 45 for 10 species, 4950 for 100
...
\footnotesize
```{r,eval=FALSE}
glmmVA(abundance ~ species + x:species + (0+species|sites), data = data, family = "poisson")
```
\normalsize
...
\item jSDM (Bayesian, relatively slow, but faster than Boral)
...
\textcolor{red}{Which software is most suitable depends on your aim, data type, and model.}
...
```{r cors1, echo = FALSE, dev = "png", dpi = 150, fig.height = 10}
cors <- getResidualCor(model1)
...
## Example: adding an environmental variable

\footnotesize

Now that we have incorporated associations, we can add in environmental variables as yesterday (fixed or random):

```{r cors2, warning=FALSE, cache = TRUE, echo = -c(2,3), dev = "png", fig.show="hide", fig.height = 10} 
...
```{r cors3, cache = TRUE, echo = -c(2,3),dev = "png", fig.show="hide", warning = FALSE, dpi = 150, fig.height = 10}
...
\columnsbegin
\column{0.3\textwidth}
\includegraphics[width=\linewidth]{JSDM_files/figure-beamer/cors1-1.png}
\column{0.3\textwidth}
\includegraphics[width=\linewidth]{JSDM_files/figure-beamer/cors2-1.png}
\column{0.3\textwidth}
\includegraphics[width=\linewidth]{JSDM_files/figure-beamer/cors3-1.png}
\columnsend
...
We represent the associations as:

\begin{equation}
\symbf{\Sigma} \approx \symbf{\Gamma}\symbf{\Gamma}^\top
\end{equation}

The number of columns in $\symbf{\Gamma}$ determines the number of latent variables.

\begin{itemize}
  \item More latent variables: better approximation of $\symbf{\Sigma}$, but slower to fit
  \item How many to use is a trade-off, evaluated by:
  \begin{itemize}
    \item Information criteria or likelihood ratio tests
    \item Variation explained
    \item Cross-validation or predictive performance
  \end{itemize}
\end{itemize}
...
## Link functions: cloglog

In cloglog regression, we again introduce a latent variable $\eta^\star_i$ for thresholding. \pause

Probit regression: $\epsilon \sim \mathcal{N}(0,1)$ \newline
Logistic regression: $\epsilon \sim \mathcal{Logistic}(0,1)$ \newline
Cloglog regression: $\epsilon \sim \mathcal{Gumbel}(0,1)$
...
# Gumbel (type I extreme value) density
dgumbel <- function(x) exp(x - exp(x))

eta <- -1
x <- seq(-5, 5, length.out = 1000)

plot(x, dgumbel(x), type="l",
     ylab="density",
     xlab=expression(eta[i]^"*"))

axis(1, at = c(-4,-2,-1,0,1,2,4))

polygon(c(x[x<eta], rev(x[x<eta])),
        c(rep(0, sum(x<eta)), rev(dgumbel(x[x<eta]))),
        col="red")
```
...
\begin{aligned}
p_i &= 1 - \exp\{-\exp(\eta_i)\} \\
\eta_i &= \mathbf{x}_i^\top \symbf{\beta} \\
\eta_i^\star &= \mathbf{x}_i^\top \symbf{\beta} + \epsilon_i,\quad \epsilon_i \sim \mathcal{Gumbel}(0,1)
\end{aligned}
\end{equation}
...
## Example: fit JSDMs and check performance
...
```{r ex2cors, cache = TRUE, echo = -c(4:8), dev="png", fig.height = 10, dpi = 150} 
goodnessOfFit(Y, object = model5, measure = "AUC")$AUC
goodnessOfFit(Y, object = model6, measure = "AUC")$AUC
goodnessOfFit(Y, object = model7, measure = "AUC")$AUC
R <- getResidualCor(model5)
ord <- corrplot::corrMatOrder(R, order = "AOE")
corrplot::corrplot(getResidualCor(model5)[ord,ord], type = "lower", diag = FALSE, tl.pos = "l", tl.cex = 0.2, order = "original")
corrplot::corrplot(getResidualCor(model6)[ord,ord], type = "lower", diag = FALSE, tl.pos = "l", tl.cex = 0.2, order = "original")
corrplot::corrplot(getResidualCor(model7)[ord,ord], type = "lower", diag = FALSE, tl.pos = "l", tl.cex = 0.2, order = "original")
```
...
\columnsbegin
\column{0.3\textwidth}
\includegraphics[width=\linewidth]{JSDM_files/figure-beamer/ex2cors-1.png}
\column{0.3\textwidth}
\includegraphics[width=\linewidth]{JSDM_files/figure-beamer/ex2cors-2.png}
\column{0.3\textwidth}
\includegraphics[width=\linewidth]{JSDM_files/figure-beamer/ex2cors-3.png}
\columnsend
