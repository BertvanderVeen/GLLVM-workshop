...
\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{macrozoobenthos.jpg}
\caption{nioz.nl}
\end{figure}
\columnsend
...
\columnsbegin
\column{0.5\textwidth}
\includegraphics[width=\linewidth]{Conditioning_files/figure-beamer/gord1-1.png}
This is an ordination at the island-level.
\column{0.5\textwidth}
\includegraphics[width=\linewidth]{Conditioning_files/figure-beamer/gord1-2.png}
These are two ordinations with the same loadings.
\columnsend

## Shared co-occurrence pattern across scales

Both terms in \texttt{\textasciitilde(1|island) + (1|transect)} share the same loadings $\symbf{\Gamma}$:

$$\text{Cov}(\mathbf{y}_i) = \sigma^2_\text{island}\,\symbf{\Gamma}\symbf{\Gamma}^\top + \sigma^2_\text{transect}\,\symbf{\Gamma}\symbf{\Gamma}^\top$$

The co-occurrence \textbf{pattern} is the same at both levels; only the \textbf{variance} differs.

\pause

Hmsc estimates a separate $\symbf{\Omega}_\text{island}$ and $\symbf{\Omega}_\text{transect}$, allowing the pattern to differ between scales.

\pause

The gllvm assumption is defensible: if \textit{Corophium} and \textit{Nereis} co-occur due to shared sediment preferences, that relationship does not change sign between the transect and island scale. What changes is how much unexplained variation exists at each level, not which species go together.
...
\columnsbegin
\column{0.5\textwidth}
\includegraphics[width=\linewidth]{Conditioning_files/figure-beamer/gord2-1.png}
\column{0.5\textwidth}
\includegraphics[width=\linewidth]{Conditioning_files/figure-beamer/gord2-2.png}
\columnsend
...
\footnotesize Combining \texttt{lvCor} with \texttt{num.RR} is more prone to convergence issues than either alone; check the optimizer warnings and consider increasing \texttt{n.init} if you see them.\normalsize
...
\tikzmarknode{t2}{\highlight{blue}{\beta_{0j}}}
+
\tikzmarknode{t4}{\highlight{grey}{\textbf{z}_k^\top\symbf{\lambda}_j}}
+
\tikzmarknode{t6}{\highlight{yellow}{\textbf{u}_i^\top}}
\tikzmarknode{t7}{\highlight{green}{\symbf{\gamma}_j}}
\end{equation}
...
$\eta_{ij} =\beta_{0j} + \textbf{x}_i^{\text{soil type}\top}\symbf{\beta}_j^\text{soil type} + \textbf{x}_i^{\text{fertilizer}\top}\symbf{\beta}_j^\text{fertilizer}$
...
\begin{equation}
\eta_{ij} = \beta_{0j} + \textbf{x}_i^{\text{soil type}\top}\symbf{\beta}_j^\text{soil type} + \textbf{u}_i^\top\symbf{\gamma}_j
\end{equation}
...
```{r alpinemap, message = FALSE, cache = TRUE, echo = FALSE, fig.height = 4, warning=FALSE}
library(dplyr)
options(HTTPUserAgent = "GLLVM-workshop (https://github.com/BertvanderVeen/GLLVM-workshop)")
swiss = rnaturalearth::ne_countries(country = 'switzerland', scale = 'large', returnclass = "sf") %>% sf::st_transform("EPSG:21781")
pts <- sf:::st_as_sf(XYraw, coords=c("X","Y"),crs = "EPSG:21781")
...
model4 <- gllvm(Y, num.lv = 2, family = "binomial", sd.errors = FALSE, diag.iter = 0)
...
\includegraphics[width=\linewidth]{Conditioning_files/figure-beamer/alpord1-1.pdf}
\column{0.34\textwidth}
\includegraphics[width=\linewidth]{Conditioning_files/figure-beamer/alpord1-2.pdf}
\column{0.34\textwidth}
\includegraphics[width=\linewidth]{Conditioning_files/figure-beamer/alpord1-3.pdf}
\columnsend
...
\includegraphics[height=0.4\textheight, width=\linewidth]{saxifraga_oppositifolia.jpg}
};
\end{figure}
\column{0.25\textwidth}
\begin{figure}
...
\includegraphics[height=0.4\textheight, width=\linewidth]{trifolium_repens.jpg}
};
\end{figure}
\column{0.25\textwidth}
\begin{figure}
...
\caption{\tiny Anthoxanthum odoratum}
 \tikz[remember picture,baseline] \node[inner sep=0pt] (anthodor){
  \includegraphics[height=0.4\textheight, width=\linewidth]{anthoxanthum_odoratum.jpg}
};
\end{figure}
\column{0.25\textwidth}
\begin{figure}
...
\includegraphics[height=0.4\textheight, width=\linewidth]{scabiosa_lucida.jpg}
};
\end{figure}
\columnsend
...
\only<1->{
  \draw[->, thick, red] (anthodor) -- ++(0.4, 3.9); % relative arrow
}
\only<1->{
  \draw[->, thick, red] (saxioppo) -- ++(4.8, 5.4); % relative arrow
}
\only<1->{
  \draw[->, thick, red] (trifrepe) -- ++(1.5, 3.8); % relative arrow
}
\only<1->{
  \draw[->, thick, red] (scabluci) -- ++(-1.8, 5); % relative arrow
}
\end{tikzpicture}
...
\begin{equation}
\eta_{ij} = \beta_{0j} + \tikzmarknode{t1}{\highlight{grey}{\textbf{x}_i^\top\symbf{\beta}_j}}
\end{equation}
...
\begin{equation}
\eta_{ij} = \beta_{0j} + \tikzmarknode{t1}{\highlight{grey}{\textbf{x}_i^\top\symbf{\beta}_j}} + \highlight{yellow}{\textbf{x}_i^{lv,\top}\textbf{B}}\highlight{green}{\symbf{\gamma}_j}
\end{equation}

\pause

Now, we are conditioning our constrained ordination on $\highlight{grey}{\textbf{x}_i^\top\symbf{\beta}_j}$
...
\begin{equation}
\eta_{ij} = \beta_{0j} + \textbf{x}_i^\top\symbf{\beta}_j +  \highlight{grey}{\textbf{x}_i^\top\textbf{B}\symbf{\gamma}_j} + \textbf{u}_i^\top\symbf{\gamma}_j
\end{equation}
...
knitr::kable(head(dune, 5)[, 1:6], format="latex", booktabs = T)
```
...
Now, the arrows for Moisture 5 and Management NM have disappeared, and the groups have been pulled together.
}
\columnsend
