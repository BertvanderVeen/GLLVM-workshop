...
- Canonical Correspondence Analysis \footnotesize (CCA, ter Braak 1986)\normalsize
...
```{r cord1, fig.height = 5, dev = "png", echo = -c(2,3), warning=FALSE, fig.show = "hide"}
...
\textbf{VGLM}
\includegraphics[width=\linewidth]{OrdWithPred_files/figure-beamer/cord1-2.png}
\column{0.49\textwidth}
\textbf{RR-VGLM}
\includegraphics[width=\linewidth]{OrdWithPred_files/figure-beamer/cord1-1.png}
\columnsend

# Constrained ordination: extensions

## Constrained ordination

- Species effects can be retrieved for any covariate
- Extreme results occur, usually due to insufficient data
- GLLVMs pick up on extreme clustering -very- well
...
gllvm::ordiplot(model3, type= "conditional", rotate = FALSE, symbols = TRUE)
gllvm::ordiplot(model3, type = "marginal", rotate = FALSE, symbols = TRUE)
gllvm::ordiplot(model3, type = "residual", rotate = FALSE, symbols = TRUE)
```
...
## Example 3: random canonical coefficients
...
The `lv.formula` argument follows standard random-effects style; effects in the same brackets share a correlation structure.
...
layout(matrix(c(1,1,2,3), 2, 2, byrow=TRUE))
gllvm::ordiplot(model4, type= "conditional", rotate = FALSE, symbols = TRUE)
gllvm::ordiplot(model4, type = "marginal", rotate = FALSE, symbols = TRUE)
gllvm::ordiplot(model4, type = "residual", rotate = FALSE, symbols = TRUE)
```

## Example 3: Random coefficient plot of species

```{r cnord2_rancoef, cache = TRUE, warning = FALSE, message = FALSE, fig.height = 2.5, fig.width = 5.5}
gllvm::randomCoefplot(model4, which.Xcoef="elevation", y.label=FALSE, mar=c(3,1,1,1))
```
